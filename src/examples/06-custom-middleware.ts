...
import { DateTime, Effect, Layer, Option, Sink } from 'effect';
import {
  CrawlResult,
  makeSpiderConfig,
  SpiderConfig,
...
const TimingMiddleware: SpiderMiddleware = {
  processRequest: (request: SpiderRequest) =>
    Effect.gen(function* () {
      ...
      const enhancedRequest = request.withHeaders({
  ...
  processResponse: (response: SpiderResponse, request: SpiderRequest) =>
    Effect.gen(function* () {
      const startTimeStr = request.headers.pipe(
        Option.flatMap((h: Record<string, string>) =>
          Option.fromNullishOr(h['X-Request-Start'])
        ),
        Option.getOrElse(() => '0')
      );
  ...
  processResponse: (response: SpiderResponse, request: SpiderRequest) =>
    Effect.gen(function* () {
      ...
      if (statusCode === 200) {
        ...
        yield* Effect.logInfo(
          `   Words: ${wordCount}, Links: ${linkCount}, Images: ${imageCount}`
        );
      }
...
const program = Effect.gen(function* () {
  ...
  yield* Effect.logInfo(
    'Demonstrating middleware pipeline and custom processing\n'
  );
    ...
    (result: CrawlResult) =>
      Effect.gen(function* () {
        ...
        if (CrawlResult.isOk(result)) {
          ...
          yield* Effect.logInfo(
            `  Processing time: ${result.pageData.scrapeDurationMs}ms`
          );

          middlewareStats.totalProcessingTime +=
            result.pageData.scrapeDurationMs;

          // Check for middleware-added headers (if any)
          if (result.pageData.headers) {
            const customHeaders = Object.keys(result.pageData.headers).filter(
              (key) =>
                key.startsWith('x-spider-') || key.startsWith('X-Spider-')
            );

            if (customHeaders.length > 0) {
              yield* Effect.logInfo(
                `  Custom headers: ${customHeaders.join(', ')}`
              );
            }
          }
        } else {
  ...
  yield* Effect.logInfo(
    '  1. TimingMiddleware - Request timing and custom headers'
  );
  yield* Effect.logInfo(
    '  2. PatternDetectionMiddleware - URL pattern analysis'
  );
  yield* Effect.logInfo(
    '  3. ContentAnalysisMiddleware - Response content analysis'
  );
  yield* Effect.logInfo('  4. Built-in RateLimitMiddleware - Rate limiting');
  yield* Effect.logInfo(
    '  5. Built-in LoggingMiddleware - Request/response logging'
  );
  yield* Effect.logInfo(
    '  6. Built-in UserAgentMiddleware - User agent headers'
  );
  yield* Effect.logInfo(
    '  7. Built-in StatsMiddleware - Statistics collection\n'
  );
  ...
  yield* Effect.logInfo(
    '   - PatternDetectionMiddleware: URL pattern analysis'
  );
  yield* Effect.logInfo(
    '   - ContentAnalysisMiddleware: Response content analysis'
  );
  ...
  yield* Effect.logInfo(
    `- Total requests processed: ${middlewareStats.requestsProcessed}`
  );
  yield* Effect.logInfo(`- Total crawl time: ${duration.toFixed(2)}s`);
  const avgTime =
    middlewareStats.requestsProcessed > 0
      ? (
          middlewareStats.totalProcessingTime /
          middlewareStats.requestsProcessed
        ).toFixed(0)
  ...
  yield* Effect.logInfo(
    `- Errors handled by middleware: ${middlewareStats.errorsHandled}`
  );
...
const mainEffect = program.pipe(
  ...
  Effect.tap((stats) =>
    Effect.gen(function* () {
      ...
      yield* Effect.logInfo(
        `Demonstrated: Custom middleware, built-in middleware, request/response processing`
      );
      yield* Effect.logInfo(
        `Processed ${stats.requestsProcessed} requests with comprehensive middleware pipeline`
      );
    })
  ),
